...
// SoundUX-style "Listen to this device" VB-Cable routing
// These PROPERTYKEYs control Windows Sound Control Panel -> Recording Device -> Listen tab
...
static bool ConfigureVBCableRouting()
{
    ...
    // Find all capture devices and configure "Listen to this device" on the real mic
    ...
    for (UINT i = 0; i < captureCount; ++i)
    {
        ...
        // Skip VB-Cable itself and other virtual devices
        ...
        if (SUCCEEDED(dev->OpenPropertyStore(STGM_WRITE, &store)))
        {
            ...
            // Set "Playback through" to VB-Cable
...
bool ParseHotkey(const std::wstring& keyStringIn, UINT &outModifiers, UINT &outVk)
{
    ...
    // parse modifiers like CTRL+, ALT+, SHIFT+
    size_t pos = 0;
    while (true)
    {
...
bool RegisterSlotHotkey(HWND hwnd, int slot, const std::wstring& keyString)
{
    ...
    // Prevent registering plain alphanumeric or SPACE keys without modifiers
...
// Inject the bound key for a slot (simulate press+release) while temporarily
// unregistering the hotkey to avoid retriggering.
...
void PopulateOutputDeviceList()
{
    ...
    for (int i = 0; i < deviceCount; ++i)
    {
        ...
        if (waveOutGetDevCapsW(i, &caps, sizeof(caps)) == MMSYSERR_NOERROR)
        {
            ...
            // Look for VB-Cable (Virtual Audio Cable)
    ...
    // Select VB-Cable if found, otherwise select first device
    if (vbCableIndex >= 0)
    {
    ...
    else
    {
        ...
        // VB-Cable not found - prompt user to install it
...
void StopSlotSound(int index)
{
    ...
    // If playing via waveOut (streaming or PCM), stop and clean up
    if (gSlots[index].hWaveOut != nullptr)
    {
...
bool PlayPcmData(int index, const WAVEFORMATEX& wf, std::vector<BYTE>&& data)
{
    ...
    // waveOut to default device (speakers fallback)
...
void PlaySlotSound(int index)
{
    ...
    // Try WAV PCM/streaming first (routes to selected output device via waveOut)
    ...
    // MP3: decode via minimp3 to PCM, mix into mic (VB-Cable) + play MCI to speakers
    if (fileType == L"mpegvideo")
    {
        ...
        // Play MCI to speakers FIRST (before miniaudio locks the file)
        ...
        // Then play to VB-Cable via miniaudio
        PlaySoundToVBCable(gSlots[index].path.c_str());

        return;
    }

    // MCI fallback for other types (plays to default device)
...
// Forward declarations for helpers used in WindowProc
                            ...
                            WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        else if (id >= KEYBIND_SET_BUTTON_ID_BASE && id < KEYBIND_SET_BUTTON_ID_BASE + SLOT_COUNT)
        {
            ...
            // Enter interactive capture mode: user presses desired combo (e.g., Alt+1)
    ...
    case WM_HOTKEY:
    {
        ...
        {
            PlaySlotSound(slot);
            // Also inject the key so the original typing/shortcut happens
                    ...
                    int nCmdShow)
{
    ...
    // Output device label + combo (changeable, auto-selects VB-Cable)
...
// Helper: format a hotkey string from modifiers and vk
std::wstring FormatHotkeyString(UINT modifiers, UINT vk)
{
    std::wstring out;
...
// Interactive capture: waits for next key (with modifiers) and registers it
void CaptureHotkeyInteractive(HWND hwnd, int slot)
{
    SetWindowTextW(gKeybindEdits[slot], L"Press key combination... (Esc to cancel)");
    // Ensure main window receives keyboard messages
    SetFocus(hwnd);

    MSG msg{};
    BOOL messageResult = 0;
    // We'll bind when the non-modifier key is released (key-up), so user can hold modifiers then press a key and release it.
    while ((messageResult = GetMessage(&msg, nullptr, 0, 0)) > 0)
    {
        if (msg.message == WM_KEYDOWN || msg.message == WM_SYSKEYDOWN)
        {
            ...
            // otherwise keep waiting for key-up
        }
        ...
        if (msg.message == WM_KEYUP || msg.message == WM_SYSKEYUP)
        {
            ...
            // ignore pure modifier key releases; wait for a non-modifier key release
            if (IsModifierKey(vk))
            {
                // continue waiting
            }
            else
            {
                ...
                // If user released a plain alphanumeric or space without modifiers, show guidance and cancel
                if (IsPlainTypingHotkey(modifiers, vk))
                {
